...
async fn main() -> io::Result<()> {
    ...
    let my_address = "10.7.16.113:8082"; // da address el Server
    let peers = vec![
        "10.7.17.170:2010",  // Address of Peer 1
        "10.7.19.179:8084",  // Address of Peer 2
    ];
    ...
    let socket6 = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind("10.7.16.113:2002").await?));

    let socket_election = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind("10.7.16.113:8083").await?)); // server-server socket
    let socketsendipback = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind("10.7.16.113:8086").await?)); // my socket for sending back my socket

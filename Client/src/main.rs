...
async fn main() -> io::Result<()> {
    ...
    loop {
        ...
        println!("Do you want to start Sending Message? (y/n): "); // added this for testing
        io::stdin()
            .read_line(&mut input)
        ...
        loop {
            ...
            let receive_result =
                timeout(Duration::from_secs(5), socket.recv_from(&mut buffer)).await;
        ...
        let format =
            image::guess_format(&std::fs::read(input).expect("Failed to read the image file"))

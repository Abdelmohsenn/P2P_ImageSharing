...
async fn main() -> io::Result<()> {
    ...
    let my_address = "10.7.17.170:8082"; // da address el Server
    let peers = vec![
        "10.7.19.179:8084",  // Address of Peer 1
        "10.7.16.113:8083",  // Address of Peer 2
    ];
    let client_address = "10.40.51.162:8080"; // da for sending back my socket
    let socket_client = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind(my_address).await?));
    let socket6 = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind("10.7.17.170:2002").await?));

    let socket_election = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind("10.7.17.170:2010").await?)); // server-server socket
    let socketsendipback = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind("10.7.17.170:8086").await?)); // my socket for sending back my socket

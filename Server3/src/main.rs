...
async fn main() -> io::Result<()> {
    ...
    let socket_election = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind("127.0.0.1:2010").await?)); // server-server socket
    let socketsendipback = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind("127.0.0.1:2014").await?));
    let socket6 = Arc::new(tokio::sync::Mutex::new(UdpSocket::bind("127.0.0.1:2004").await?));
